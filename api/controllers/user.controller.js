  ...
  try {
    ...
    if (!isAdmin) {
      return res.status(403).json({
        status: "failed",
        message: "Unauthorized. You need higher permissions to do that",
      });
    }
...
// search users (by dateAdded, firstName, lastName)
export const getUsersWithSearch = asyncHandler(async (req, res, next) => {
  try {
    const { dateAdded, firstName, lastName } = req.query;
    const filter = {};

    if (dateAdded) {
      filter.createdAt = { $gte: new Date(dateAdded) };
    }
    ...
    return res.status(200).json({
      status: "success",
      message: "fetching Users with filtering successful",
      users,
    });
  } catch (error) {
